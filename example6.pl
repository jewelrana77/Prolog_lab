% Facts
current_temperature(Temp) :- Temp = 30.   % Example: current temperature is 30°C.
desired_temperature(Temp) :- Temp = 22.   % Example: desired temperature is 22°C.
...
% Home occupancy
home_occupied(yes).  % Example: home is currently occupied (could also be 'no').
